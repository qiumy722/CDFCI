...
int main(int argc, char *argv[])
{
    ...
    hamiltonian.init_HamiltonianHubbardK(1.0, U, lattice, 14, 0);
    std::vector<double> orbital_energy = hamiltonian.orbital_energy;

    // Base determinant has 10 orbitals occupied.
    ...
    for (auto orb : orbitals)
        std::cout << orb << " ";
    std::cout << std::endl;
    // Output: Occupied orbitals: 0 1 2 3 6 7 8 9 24 25
    std::cout << "Alpha orbitals at zero energy: ";
    for (auto orb : alpha_orbitals)
        std::cout << orb << " ";
    std::cout << std::endl;
    // Output: Alpha orbitals at zero energy: 4 10 14 16 26 30
    std::cout << "Beta orbitals at zero energy: ";
    for (auto orb : beta_orbitals)
        std::cout << orb << " ";
    std::cout << std::endl;
    // Output: Beta orbitals at zero energy: 5 11 15 17 27 31

    // Fill in the alpha and beta orbitals with 4 electrons, while keeping spin = 0.
    ...
    // Output: Total determinants generated: 400
    ...
    save_txt(A, "hubbard_4x4_n14_Hamiltonian_matrix.txt");
    ...
    WaveFunction<ContainerRobinhood<Determinant<N>, std::array<NumericalType, 2>,
        DeterminantHash<N>, DeterminantEqual<N>>, N> wavefunction;
    for (int i = 0; i < n; ++i)
    {
    ...
    wavefunction.dump_wavefunction("hubbard_4x4_n14_init.dat");
